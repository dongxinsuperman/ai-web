...
>
> 关键原则：**对外接口清单不改，case-flow 调用无感。** Agent、节点、Windows 主机、浏览器数量都是 AI Web 内部实现细节。
...
`/api/devices/*` 仍然保持“浏览器槽”视角，用于展示可用性，不作为 case-flow 必须理解的派发入口。
...
```text
case-flow -> POST /api/submissions
